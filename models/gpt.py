from forecast.utils import serialize_arr, SerializerSettings
import openai
import tiktoken
import numpy as np
...
def gpt_completion_fn(model, input_str, steps, settings, num_samples, temp):
    ...
    """
    print(f"载入gpt_completion的model参数: {model}")
...
#添加的模型名字修正
    ...
    if model in ['gpt-3.5-turbo','gpt-4','gpt-4-1106-preview',"deepseek-ai/DeepSeek-R1-Distill-Llama-8B"]:
