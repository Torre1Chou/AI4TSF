...
def sample_completions(model, input_str, steps, settings, num_samples, temp, logit_bias, **kwargs):
    ...
    if model in ['gpt-3.5-turbo', 'gpt-4', 'deepseek-ai/DeepSeek-R1-Distill-Llama-8B']:
        chatgpt_sys_message = "你是一个用于时间序列预测的助手。用户将提供一个序列，你需要预测剩余的序列。序列由逗号分隔的十进制字符串表示。"
        extra_input = "请继续以下序列，不要生成任何额外的文本。不要使用类似'序列的下一个项是'的语句，只需返回数字。序列:\n"
...
def generate_predictions(
    ...
    **kwargs,
    ...
    # 设置logit偏置
    logit_bias = {}
    if (model not in ['gpt-3.5-turbo', 'gpt-4']) and constrain_tokens:  # chat模型不支持logit偏置
...
def get_promptcast_predictions_data(train, test, model, settings, num_samples=10, temp=0.8, dataset_name='dataset', parallel=True, **kwargs):
    """
    ...
    # 设置提示文本
    prompt = f'{dataset_name} 数据集过去 {len(train[0])} 个时间步的值为 '
